...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    ...
    long counterA = 0, counterO = 0;
    long aD = s - a; // d positive for apples
    long oD = t - b; // d negative for oranges
    sort(apples.begin(), apples.end());
    sort(oranges.begin(), oranges.end());
    for (size_t i = 0; i < apples.size(); ++i) {
        if (apples.at(i) >= aD &&  apples.at(i) <= aD + t - s) {
            counterA++;
        }
    }

    for (size_t j = 0; j < oranges.size(); ++j) {
        if (oranges.at(j) <= oD && oranges.at(j) >= oD - (t-s)) {
            counterO++;
        }
    }
